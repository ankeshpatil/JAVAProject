...
//		list.stream().sorted((a,b)->(b.getSalary())-(a.getSalary())).map(e->e.getSalary()).forEach(System.out::println);
		...
		//Sort employ salary greater than 20000 and add 10000 in the filtered salary
...
//liii.forEach(e->{System.out.println("elements are  "+e);});
//	
...
//
 ...
 //Reversing/sorting the order
// ai.stream().sorted(Comparator.reverseOrder()).forEach(System.out::println);
...
// List<Iteger>ak=ai.stream().distinct().collect(Collectors.toList());
// System.out.println(ak);

//In map getting duplicates occurrence in the form of key value pair and printing duplicate element key 
...
// .entrySet().stream().filter(e->e.getValue() >1).map(Map.Entry::getKey).forEach(System.out::println);
 ...
 //Printing occurrence of value in key pair
